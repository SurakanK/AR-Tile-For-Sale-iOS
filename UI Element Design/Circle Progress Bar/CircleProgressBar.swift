...
//  AR Tile
//
//  Created by พัทธนันท์ ปุ่นน้ำใส on 24/2/2563 BE.
...
open class CircleProgressBar: UIView {
    ...
    override init(frame: CGRect) {
        ...
        //MARK: - Pulse Layer
        pulseLayer = createCircleShapeLayer(strokeColor: .clear, fillColor: pulseColor, frame: frame)
        self.layer.addSublayer(pulseLayer)
        startPulseAnimation()
        
        //MARK: - Track Layer
        trackLayer = createCircleShapeLayer(strokeColor: trackStrokeColor, fillColor: trackFillColor, frame: frame)
        self.layer.addSublayer(trackLayer)
        
        //MARK: - Shape Layer
        shapeLayer = createCircleShapeLayer(strokeColor: progressColor, fillColor: .clear,frame: frame)
        shapeLayer.transform = CATransform3DMakeRotation(-CGFloat.pi / 2, 0, 0, 1)
        shapeLayer.strokeEnd = 0
        self.layer.addSublayer(shapeLayer)
        setupProgressLabel(frame: frame)
    }
    ...
    private func createCircleShapeLayer(strokeColor: UIColor, fillColor: UIColor, frame : CGRect) -> CAShapeLayer {
    ...
    private func setupProgressLabel(frame : CGRect) {
        self.addSubview(progressLabel)
        progressLabel.frame = CGRect(x: 0, y: 0, width: frame.width / 4, height: frame.height / 4)
        progressLabel.center = CGPoint(x: frame.width / 2, y: frame.height / 2)
    }
